...
const MyOrder = () => {
  ...
  const {
    ...
    queryFn: () =>
      fetch(`https://aptdeco.vercel.app/bookings/${user?.email}`, {
        method: "GET",
        headers: {
  ...
  const handleDelete = (id) => {
    fetch(`https://aptdeco.vercel.app/bookings/${id}`, {
      method: "DELETE",
      headers: {
        "content-type": "application/json",
        authorization: `Bearer ${localStorage.getItem("token")}`,
                  ...
                  <tbody>
                    {Orders?.map((order, i) => (
                      <tr key={order?._id}>
                        ...
                        <td className="font-bold">$ {order?.itemPrice}</td>
                        <td>
                          {order?.paid === "false" ? (
